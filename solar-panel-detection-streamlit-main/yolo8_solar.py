...
def process_excel_file(excel_path, output_dir='output'):
    ...
    Args:
        excel_path: Path to Excel file with columns: S.No, Latitude, Longitude
    ...
    original_dir = output_path / zoom_21 /'original'
    detected_dir = output_path /zoom_21 / 'detected'
    ...
    required_cols = ['S.No', 'Latitude', 'Longitude']
    ...
    for idx, row in df.iterrows():
        s_no = row['S.No']
        lat_input = row['Latitude']
        lon_input = row['Longitude']
        
        print(f"\n{'='*60}")
        print(f"Processing row {idx+1}/{len(df)} - S.No: {s_no}")
        ...
        if lat is None or lon is None:
            print(f"⚠️  Invalid coordinates for S.No {s_no}, skipping...")
            results.append({
                'S.No': s_no,
                'Latitude': lat_input,
                'Longitude': lon_input,
        ...
        if img is None:
            print(f"⚠️  Failed to fetch image for S.No {s_no}, skipping...")
            results.append({
                'S.No': s_no,
                'Latitude': lat,
                'Longitude': lon,
        ...
        # Save original image
        original_filename = f"{s_no}_original.jpg"
        ...
        # Save detected image
        detected_filename = f"{s_no}_detected.jpg"
    ...
    results_csv = output_path / 'detection_results.xlsx'
...
if __name__ == "__main__":
    ...
    if not os.path.exists(excel_file):
        ...
        print("\nUsage:")
        print("  python yolo8_solar.py <excel_file> [output_directory]")
        print("\nExample:")
        print("  python yolo8_solar.py coordinates.xlsx output")
        print("\nThe Excel file should contain columns: S.No, Latitude, Longitude")
        sys.exit(1)
