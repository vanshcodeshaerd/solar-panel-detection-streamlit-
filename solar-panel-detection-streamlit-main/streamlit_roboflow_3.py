...
def create_segmentation_visualization(image, predictions):
    ...
    for pred in predictions.get("predictions", []):
        ...
        # Try different possible field names for segmentation data
        if "segmentation" in pred and pred["segmentation"]:
            segmentation_data = pred["segmentation"]
        elif "mask" in pred and pred["mask"]:
            segmentation_data = pred["mask"]
        elif "polygon" in pred and pred["polygon"]:
            segmentation_data = pred["polygon"]
        elif "contour" in pred and pred["contour"]:
            segmentation_data = pred["contour"]
        elif "points" in pred and pred["points"]:
            segmentation_data = pred["points"]
        elif "coordinates" in pred and pred["coordinates"]:
            segmentation_data = pred["coordinates"]
        
        # Debug: Print the prediction to see what's available
        print(f"Prediction keys: {pred.keys()}")
        if "segmentation" in pred:
            print(f"Segmentation data type: {type(pred['segmentation'])}")
            print(f"Segmentation data: {pred['segmentation']}")
...
if mode == "Upload Image":
    ...
    if uploaded_file:
        ...
        with col2:
            ...
            # Debug: Show prediction structure
            st.write("**Debug Info:**")
            if preds and "predictions" in preds and len(preds["predictions"]) > 0:
                first_pred = preds["predictions"][0]
                st.write(f"Available keys: {list(first_pred.keys())}")
                if "segmentation" in first_pred:
                    st.write(f"Segmentation data type: {type(first_pred['segmentation'])}")
                    st.write(f"Segmentation data: {first_pred['segmentation']}")
