...
if not ROBOFLOW_API_KEY:
    st.error("⚠️ ROBOFLOW_API_KEY not found. Please add it to .streamlit/secrets.toml")
    st.stop()

try:
    rf = Roboflow(api_key=ROBOFLOW_API_KEY)
    # Try to get the project directly without workspace()
    project = rf.workspace().project(os.getenv("ROBOFLOW_PROJECT", ""))
    model = project.version(8).model
except AttributeError:
    # Fallback: use direct workspace name if workspace() fails
    rf = Roboflow(api_key=ROBOFLOW_API_KEY)
    project = rf.workspace("sheetalbishtphd-hgwco").project(os.getenv("ROBOFLOW_PROJECT", ""))
    ...
    if uploaded_file:
        ...
        with col2:
            ...
            if detection_count > 0:
                st.markdown(f'<div class="detection-counter">✅ Solar panels installed: {detection_count}</div>', unsafe_allow_html=True)
            else:
                st.markdown('<div class="detection-counter">❌ No solar panels installed</div>', unsafe_allow_html=True)
