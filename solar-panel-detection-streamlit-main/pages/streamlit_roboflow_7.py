...
# --- Config: prefer .env, then session (in-app paste), then Streamlit secrets ---
...
if not GOOGLE_API_KEY or GOOGLE_API_KEY.startswith("your_"):
    try:
        ...
        pass
ROBOFLOW_PROJECT = (os.getenv("ROBOFLOW_PROJECT") or "solarpv-india-lczsp").strip()
if not ROBOFLOW_PROJECT:
    try:
        ROBOFLOW_PROJECT = (st.secrets.get("ROBOFLOW_PROJECT") or "solarpv-india-lczsp").strip()
    except Exception:
        ROBOFLOW_PROJECT = "solarpv-india-lczsp"
ROBOFLOW_WORKSPACE = (os.getenv("ROBOFLOW_WORKSPACE") or "").strip()
if not ROBOFLOW_WORKSPACE:
    try:
        ROBOFLOW_WORKSPACE = (st.secrets.get("ROBOFLOW_WORKSPACE") or "").strip()
    except Exception:
        pass
...
def download_google_satellite(lat, lon, zoom=20, size="640x640"):
    ...
    if not gkey or gkey.startswith("your_"):
        st.error("Set GOOGLE_API_KEY in .env or .streamlit/secrets.toml to use Latitude/Longitude.")
        return None
    url = f"https://maps.googleapis.com/maps/api/staticmap?center={lat},{lon}&zoom={zoom}&size={size}&maptype=satellite&key={gkey}"
