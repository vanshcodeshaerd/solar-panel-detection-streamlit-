"""
Solar Panel Detection Script - Zoom Level 21
Reads coordinates from Excel file and detects solar panels using Roboflow model
"""
...
def download_google_satellite(lat, lon, zoom=21, size="640x640"):
...
def process_excel_file(excel_path, output_dir='output'):
    """
    Process Excel file with coordinates and detect solar panels
    
    Args:
        excel_path: Path to Excel file with columns: Name, New Latitude, New Longitude
    ...
    # Create zoom_21 subdirectory
    zoom_21_dir = output_path / 'zoom_21'
    zoom_21_dir.mkdir(exist_ok=True)
    
    # Create subdirectories for original and detected images inside zoom_21
    original_dir = zoom_21_dir / 'original'
    detected_dir = zoom_21_dir / 'detected'
    ...
    required_cols = ['Name', 'New Latitude', 'New Longitude']
    ...
    for idx, row in df.iterrows():
        name = row['Name']
        lat_input = row['New Latitude']
        lon_input = row['New Longitude']
    ...
    results_csv = zoom_21_dir / 'detection_results.csv'
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        excel_file = "/Users/manish/Desktop/Sheetal/office_work/cursor/solar_roboflow/excel/zoom_21.xlsx"
    ...
    print("="*60)
    print("Solar Panel Detection System - Zoom Level 21")
    print("="*60)
    print(f"Excel file: {excel_file}")
    print(f"Output directory: {output_directory}")
    print(f"Zoom level: 21")
    
    # Check if file exists
    if not os.path.exists(excel_file):
        print(f"\n❌ Error: Excel file '{excel_file}' not found!")
        print("\nUsage:")
        print("  python yolo8_solar_zoom21.py <excel_file> [output_directory]")
        print("\nExample:")
        print("  python yolo8_solar_zoom21.py zoom_21.xlsx output")
        print("\nThe Excel file should contain columns: Name, New Latitude, New Longitude")
        sys.exit(1)
    
    # Process the Excel file
    process_excel_file(excel_file, output_directory)
